...
type
  ...
  public
    ...
    FileTime : TDateTime;
  public
    constructor Create( _FileName : string; _IsFolder : Boolean );
...
constructor TFileInfo.Create(_FileName: string; _IsFolder: Boolean);
begin
  FileName := _FileName;
  IsFolder := _IsFolder;
end;
...
begin
  ...
  for i := FileList.Count - 1 downto 0 do
    if FileList[i].IsFolder or ( CompareText( FileName, FileList[i].FileName ) > 0 ) then
    begin
      InsertPos := i + 1;
      Break;
    end;
    ...
    // 添加
  FileInfo := TFileInfo.Create( FileName, False );
...
begin
  ...
  for i := 0 to FileList.Count - 1 do
    if not FileList[i].IsFolder or ( CompareText( FileList[i].FileName, FolderName ) > 0 ) then
    begin
      InsertPos := i;
      Break;
    end;
    ...
    // 添加
  FileInfo := TFileInfo.Create( FolderName, True );
